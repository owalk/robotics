...
MyMap::MyMap(ros::NodeHandle handle, int which_map) 
{
  for (int i = 0; i < MAP_HEIGHT; i++) 
      ...
      for (int j = 0; j < MAP_WIDTH; j++) 
	{
      ...
      // push the rows of unknowns back
      this->map.push_back(new_vector);
    }
  
  
  if(which_map == 1)
    // this map is for main occupancy grid
    this->publisher = handle.advertise<nav_msgs::OccupancyGrid>("/map", 10);
  else
    // this map holds the information of where lawnmower has cut
...
void MyMap::cut_area(int x, int y, MyMap otherMap) 
{
  for (int i = -7; i < 8; i++){ // for 15 around y
    for (int j = -7; j < 8; j++){ // for 15 around x
        if ((y+i) > 0 && (y+i) < MAP_HEIGHT && (x+j) > 0 && (x+j) < MAP_WIDTH){ // if within bounds
          if(otherMap.map[y+i][x+j] < 0.5){ // if white on main map
  ...
  - Oliver 
  */
void MyMap::add_uncut(MyMap main_map){
  for (int i = 0; i < MAP_HEIGHT; i++){
    for (int j = 0; j < MAP_WIDTH; j++){
      // if white on main map and not cut on mower map
      if( main_map.map[i][j] < 0.5 && // main_map is open space
	  main_map.map[i][j] != -1 && // main_map initialized
	  this->map[i][j] > 0.4) { // mower_map is not cut	
	this->map[i][j] = 0.9;
      } //end if
    }//end for
  }//end for
...
/*
  ...
  check for the values in our map that stores data of how much of the lawn is cut
 */
